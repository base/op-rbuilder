...
This document introduces Flashblock-Level Access Lists (FAL), an adaptation of [EIP-7928](https://eips.ethereum.org/EIPS/eip-7928) Block-Level Access Lists (BAL) for OP Stack chains that produce flashblocks. FAL records all accounts and storage locations accessed during flashblock execution, along with their post-execution values. Like BAL, FAL enables parallel disk reads, parallel transaction validation, and executionless state updates, but is specifically designed for the flashblock architecture used in OP Stack chains.

## Motivation

FAL adapts the BAL specification for OP Stack chains that produce flashblocks—incremental "mini-blocks" produced at sub-block intervals (e.g., every 200ms for a 2s canonical block time). While BAL is designed for Ethereum L1 canonical blocks, FAL must handle:
...
- `0`: Pre-execution system contracts + L1 attributes transaction
...
The FAL MUST be complete and accurate. Missing or spurious entries invalidate the flashblock.
...
Flashblock size impact (estimated):
- Average: ~45 KiB (compressed) - slightly higher than BAL due to OP Stack overhead
- L1Block updates: ~200 bytes per flashblock
- Fee vault updates: ~150 bytes per transaction (3 vaults vs 1 coinbase)
- Storage/nonce/code diffs: Similar to BAL

### Alternative Design Considerations

#### Alternative 1: Separate Index for L1 Attributes Transaction
- **Proposal**: Use separate `block_access_index` for L1 attributes tx (e.g., -1 or 0xFFFF)
- **Pros**: Clear distinction between system and user transactions
- **Cons**: More complex indexing, breaks compatibility with BAL structure
- **Decision**: Not adopted; grouping with pre-execution (index 0) is simpler

#### Alternative 2: Fee Vault Aggregation
- **Proposal**: Only record final fee vault balances at post-execution
- **Pros**: Smaller access lists
- **Cons**: Loses per-transaction fee tracking, reduces parallel validation capability
- **Decision**: Not adopted; per-transaction tracking matches BAL semantics for COINBASE

#### Alternative 3: Exclude Empty Accounts
- **Proposal**: Don't include accounts with empty change lists
- **Pros**: Smaller access lists
- **Cons**: Breaks parallel I/O capability; can't pre-fetch all accessed accounts
- **Decision**: Not adopted; including empty accounts is essential for parallelization

## Implementation Notes

### Flashblock Builder Integration

The flashblock builder must:
1. Track all state accesses during flashblock construction
2. Build FAL incrementally as transactions are added
3. Maintain `min_tx_index` and `max_tx_index` relative to the overall block
4. Compute `fal_hash` as `keccak256(rlp.encode(account_changes))` before finalizing
5. Store complete FAL structure (min_tx_index, max_tx_index, account_changes, fal_hash) in flashblock metadata

### Validator Responsibilities

Validators must:
1. Extract FAL from flashblock metadata
2. Verify `fal_hash` matches `keccak256(rlp.encode(account_changes))`
3. Execute flashblock using `min_tx_index` and `max_tx_index` to assign correct block access indices
4. Verify collected accesses match provided `account_changes`
5. Reject flashblock if FAL is incomplete or inaccurate

### Canonical Block Reconstruction

When assembling canonical blocks from flashblocks:
- Each flashblock has its own FAL
- Canonical block MAY aggregate all flashblock FALs if implementing BAL
- FAL enables validating each flashblock independently

## Security Considerations

### Validation Overhead

Validating access lists adds validation overhead but is essential to prevent acceptance of invalid flashblocks. The overhead is comparable to BAL validation.

### Flashblock Size

Increased flashblock size impacts propagation. Average overhead (~45 KiB) is reasonable for 200ms flashblock intervals and enables significant performance gains through parallelization.

### Fee Vault Tracking

Tracking three fee vaults instead of one COINBASE increases FAL size slightly but provides complete transparency into OP Stack fee distribution.
...
- [EIP-7702: Set EOA account code](https://eips.ethereum.org/EIPS/eip-7702) - Account delegation
- [EIP-2935: Save historical block hashes in state](https://eips.ethereum.org/EIPS/eip-2935) - Block hash storage
