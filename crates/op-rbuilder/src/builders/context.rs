...
/// Result of simulating a single transaction on an isolated overlay state.
#[derive(Debug, Clone)]
pub struct TxSimResult {
    /// Built receipt for the simulated transaction.
    pub receipt: OpReceipt,
    /// Gas used by the simulated transaction.
    pub tx_gas_used: u64,
    /// Cumulative gas used including sequencer transactions plus this transaction.
    pub cumulative_gas_used: u64,
    /// True if the simulated transaction executed successfully.
    pub success: bool,
    /// Total wall-clock duration of the simulation.
    pub simulation_duration: core::time::Duration,
    /// Optional duration of state root calculation if requested.
    pub state_root_calculation_duration: Option<core::time::Duration>,
}

impl OpPayloadBuilderCtx {
    /// Simulates a transaction on a fresh in-memory overlay so concurrent simulations are isolated.
    /// The base state determines where in the block timeline the simulation starts (e.g. top-of-block
    /// if it's immediately after pre-exec and sequencer txs, or any other chosen state).
    ///
    /// Expects a `Recovered<OpTransactionSigned>` for direct execution.
    pub fn simulate_transaction_with_overlay<DB, P>(
        &self,
        base_state: &State<DB>,
        tx: Recovered<OpTransactionSigned>,
        calculate_state_root: bool,
    ) -> Result<TxSimResult, PayloadBuilderError>
    where
        DB: Database<Error = ProviderError> + AsRef<P>,
        P: StateProvider + HashedPostStateProvider + StateRootProvider,
    {
        let sim_start = Instant::now();
        // Create isolated in-memory state backed by the same provider as base_state.
        let provider = base_state.database.as_ref();
        let mut sim_state = State::builder()
            .with_database(StateProviderDatabase::new(provider))
            .with_bundle_update()
            .build();

        // 1) Apply pre-execution changes for the next block.
        self
            .evm_config
        ...
        // 2) Execute sequencer (payload attribute) transactions first to reach
        //    the top-of-block state for mempool transactions.
        let pre_info: ExecutionInfo = self.execute_sequencer_transactions(&mut sim_state)?;
        // Release any borrows held by execute_sequencer_transactions before creating EVM
        // by ensuring no references to sim_state are alive here.

        // 3) Simulate the user-provided recovered transaction on top of that state, but do not commit.
        let mut evm = self.evm_config.evm_with_env(&mut sim_state, self.evm_env.clone());
        let ResultAndState { result, state } = evm
            .transact(&tx)
            .map_err(|err| PayloadBuilderError::EvmExecutionError(Box::new(err)))?;
        ...
        let cumulative_gas_used = pre_info.cumulative_gas_used + tx_gas_used;
        ...
        // Optionally compute state root and measure duration.
        let state_root_calculation_duration = if calculate_state_root {
            // Commit the simulated tx into the overlay to include it in the bundle used for state root.
            // This only affects the per-call overlay and is discarded at the end of the function.
            let mut evm = self.evm_config.evm_with_env(&mut sim_state, self.evm_env.clone());
            // Re-run a zero-cost commit by committing the previously returned state onto sim_state.
            // Note: We cannot reuse `evm` above due to borrow rules; create a new EVM handle.
            evm.db_mut().commit(state);

            // Merge transitions and compute state root using provider.
            let state_root_start_time = Instant::now();
            sim_state.merge_transitions(BundleRetention::Reverts);
            let new_bundle = sim_state.take_bundle();
            ...
            let provider = sim_state.database.as_ref();
            let hashed_state = provider.hashed_post_state(execution_outcome.state());
            // We intentionally ignore the resulting values; we only measure time.
            let _ = provider.state_root_with_updates(hashed_state);
            Some(state_root_start_time.elapsed())
        } else {
            None
        };
        ...
        Ok(TxSimResult {
            ...
            cumulative_gas_used,
            success,
            simulation_duration: sim_start.elapsed(),
