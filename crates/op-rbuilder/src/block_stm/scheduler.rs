...
pub struct TaskGuard<'a> {
    num_tasks: &'a AtomicUsize,
}

impl<'a> TaskGuard<'a> {
    fn new(num_tasks: &'a AtomicUsize) -> Self {
...
impl Scheduler {
    ...
    fn next_version_to_execute(&self) -> Option<((TxnIndex, Incarnation), TaskGuard<'_>)> {
        if self.execution_idx.load(Ordering::SeqCst) >= self.num_txns as usize {
            self.check_done();
            return None;
        }
        let task_guard = TaskGuard::new(&self.num_active_tasks);
        ...
        result.map(|version| (version, task_guard))
    }

    fn next_version_to_validate(&self) -> Option<((TxnIndex, Incarnation), TaskGuard<'_>)> {
        ...
        if idx_to_validate < self.num_txns as usize {
            let (incarnation, status) = *self.txn_status[idx_to_validate].lock().unwrap();
            if status == ExecutionStatus::Executed {
                return Some(((idx_to_validate as TxnIndex, incarnation), task_guard));
            }
        }
        None
    }

    pub fn next_task(&self) -> Option<(Task, TaskGuard<'_>)> {
        ...
        } else if let Some(((txn_idx, incarnation), guard)) = self.next_version_to_execute() {
            return Some((
                Task::Execute {
                    version: Version::new(txn_idx, incarnation),
                },
    ...
    pub fn next_validation_task(&self) -> Option<(Task, TaskGuard<'_>)> {
        if let Some(((txn_idx, incarnation), guard)) = self.next_version_to_validate() {
            return Some((
                Task::Validate {
                    version: Version::new(txn_idx, incarnation),
